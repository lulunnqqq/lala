...
source.getResource =  async (movieInfo, config, callback) => {
	...
	const decode = {
		...
		keyStr: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",
		e: function(e, t) {
            if ("e" === decode.mode) return decode.ee(e);
            if ("c" === decode.mode) return decode.ec(e, t);
            return false;
        },
        d: function(e, t) {
            if ("e" === decode.mode) return decode.de(e);
            if ("c" === decode.mode) return decode.dc(e, t);
            return false;
        },
        ee: function(e) {
        ...
        F: function(e) {
            var t = e >>> 24,
                ...
                i = e << 16 >>> 24,
                r = e << 24 >>> 24,
                o = decode.addMod32(decode.sBox0[t], decode.sBox1[n]);
            return o = decode.xor(o, decode.sBox2[i]),
                o = decode.addMod32(o, decode.sBox3[r])
        },
        encipher: function(e, t) {
            for (var n, i = 0; i < 16; i++) e = decode.xor(e, decode.pArray[i]),
        ...
        gsk: function(e) {
            var t, n, i = 0,
                r = 0;
            for (t = 0; t < 18; t++) {
                for (n = 4; n > 0; n--) i = decode.fixNegative(i << 8 | e.charCodeAt(r)),
                    r = (r + 1) % e.length;
                decode.pArray[t] = decode.xor(decode.pArray[t], i),
                    i = 0
            }
            var o = [0, 0];
            for (t = 0; t < 18; t += 2) o = decode.encipher(o[0], o[1]),
                decode.pArray[t] = o[0],
                decode.pArray[t + 1] = o[1];
            for (t = 0; t < 256; t += 2) o = decode.encipher(o[0], o[1]),
                decode.sBox0[t] = o[0],
        ...
        utf8Encode: function(e) {
            for (var t = "", n = 0, i = 0, r = 0, o = 0; n < e.length;) i = e.charCodeAt(n),
        ...
        be: function(e) {
            for (var t, n, i, r, o, a, s, u = "", l = 0; l < e.length;) t = e.charCodeAt(l++),
                n = e.charCodeAt(l++),
                i = e.charCodeAt(l++),
                r = t >> 2,
                o = (3 & t) << 4 | n >> 4,
        ...
        bd: function(e) {
            var t, n, i, r, o, a, s, u = "",
                l = 0;
            for (e = e.replace(/[^A-Za-z0-9\+\/\=]/g, ""); l < e.length;) r = decode.keyStr.indexOf(e.charAt(l++)),
	...
	let urlSearch = `https://www.primewire.li/?s=${slugify(movieInfo.title, {lower: true, replacement:'+'})}&t=y&m=m&w=q`
	...
	parseSearch(".index_item").each((keySearch, itemSearch) => {
	...
	if (link != "") {
		...
		if (movieInfo.type == "movie") {
			...
			// linkDetail = `${link}-season-${movieInfo.season}-episode-${movieInfo.episode}`

			console.log(parseTv(".show_season").length, "------------ primewire TV LINK LENGTH")
			parseTv(".show_season").each((keySeason, itemSeason) => {
				let season = parseTv(itemSeason).attr("data-id")

				console.log(season, "------------ primewire SEASON")
				if (season == movieInfo.season) {
					console.log(parseTv(itemSeason).find(".tv_episode_item").length, "------------ primewire TV EPISODE LENGTH")
					parseTv(itemSeason).find(".tv_episode_item").each((keyEpisode, itemEpisode) => {
						let hrefEpisode = parseTv(itemEpisode).find("a").attr("href")

						console.log(hrefEpisode, "------------ primewire HREF EPISODE")
						if (hrefEpisode) {
							hrefEpisode = `https://www.primewire.li${hrefEpisode}`;
							let episode = parseTv(itemEpisode).find("a").text();
		...
		console.log(linkDetail, "------------ primewire LINK DETAIL")
		if (linkDetail != "") {
			...
			let userData = parseEpisode("#user-data").attr("v");
			let sliceUserData = userData.slice(0,9)
			let sliceTermData = userData.slice(9);
			decodeGsk(sliceUserData, "e")



			const token = decode.d(decode.bd(sliceTermData));
			const resolve = token.match(/.{1,5}/g)
			...
			let arrMap = sources.map(async (item) => {
				let embed = await libs.request_get(item, {
